...
from utils.constants import IMAGE_FORMAT, DOWNSAMPLE_MODE, COLOR_CHANNELS, UPSCALING_FACTOR, HR_IMG_SIZE, LR_IMG_SIZE
...
class DIV2K_Dataset(keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i, image_fn in enumerate(batch_image_fns):
            hr_image_pil = PIL.Image.open(os.path.join(self.hr_image_folder, image_fn))
            hr_image = np.array(hr_image_pil)
            hr_image_transform = self.transform(image=hr_image)["image"]
            hr_image_transform_pil = PIL.Image.fromarray(hr_image_transform)
